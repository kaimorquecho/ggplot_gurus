...
k_plot <- ggplot() + 
  geom_smooth(data= bq1_tidy, size = 0.8, aes(x= sample_date, y = k), linetype = "dotted") + 
  geom_smooth(data= prm_tidy, size = 0.8, aes(x= sample_date, y = k ), linetype= "solid") +
  geom_smooth(data= bq2_tidy, size = 0.8, aes(x= sample_date, y = k), linetype = "dashed") +
  geom_smooth(data= bq3_tidy, size = 0.8, aes(x= sample_date, y = k), linetype = "dotdash") +
...
no3_n_plot <- ggplot() +  
  ...
  scale_y_continuous(breaks=c(5, 100, 300, 500), limits = c(5, 500)) + 
