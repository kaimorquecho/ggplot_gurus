...
k_plot <- ggplot() + 
  geom_smooth(data= bq1_tidy, aes(x= sample_date, y = k),
              linetype = "dotted", color = "black", linewidth = .8, se = FALSE, span = 0.1 ) + 
  geom_smooth(data= prm_tidy, aes(x= sample_date, y = k ),
              linetype= "solid", color = "black", linewidth = .8, se = FALSE, span = 0.1) +
  geom_smooth(data= bq2_tidy, aes(x= sample_date, y = k),
              linetype = "dashed", color = "black", linewidth = .8, se = FALSE, span = 0.1) +
  geom_smooth(data= bq3_tidy, aes(x= sample_date, y = k),
              linetype = "dotdash", color = "black", linewidth = .8, se = FALSE, span = 0.1) +
...
no3_n_plot <- ggplot() +  
  geom_smooth(data= bq2_tidy, aes(x= sample_date, y = no3_n), linetype = "dashed"
              , color = "black", linewidth = .8, se = FALSE, span = 0.1) +
